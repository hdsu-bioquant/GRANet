...
p_regulonAUCHeat     <- "figures/tfActivity_integrated/method1_regulonAUC.pdf"
p_regulonAUCbinHeat  <- "figures/tfActivity_integrated/method1_regulonAUCbin.pdf"
p_regulonAUCCorHeat  <- "figures/tfActivity_integrated/method1_regulonAUC_devCorrected.pdf"
...
#------------------------------------------------------------------------------#
p <- Heatmap_Regulon(regulon=regulonAUCbin, seurat=exprs, normRows=TRUE,
                     title = "Regulon AUC\nBinarized")
...
#------------------------------------------------------------------------------#
#                  Heatmap Motiv deviation corrected regulon AUC               #
#------------------------------------------------------------------------------#
...
regulonAUC_pySCENIC <- read_csv(p_regulonAUCpySCENIC) %>% 
  ...
  t()
saveRDS(regulonAUC_pySCENIC, p_regulonAUCpySCENICR)

p <- Heatmap_Regulon(regulon=regulonAUC_pySCENIC, seurat=exprs, normRows=TRUE,
                     title = "Regulon AUC\npySCENIC")
pdf(p_regulonAUCpyHeat, width = 10, height = 6)
p
dev.off()
...
regulonStats(regulonAUCbin)
regulonStats(regulonAUC_devCorrected)
regulonStats(regulonAUC_pySCENIC)
